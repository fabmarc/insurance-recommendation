...
it('should render without crash for empty questions', () => {
  const { getByTestId, container } = render(<Questionnaire questions={[]} answers={{}} />);
...
it('should display first question without answer for empty answers', () => {
  const { getByText, container } = render(<Questionnaire questions={[{
    "id": 1,
    "code": "firstName",
...
it('should push to history by clicking NEXT on last question', async () => {
  ...
  const { container } = render(<Questionnaire questions={[{
    "id": 1,
    "code": "firstName",
    "label": "What is your first name?"
  }]}
