...
it('should render without crash for empty response value', async () => {
  ...
  const { getByTestId } = render(<Router><Recommendation /></Router>);
...
it('should render with the response items for successful response', async () => {
  ...
  await waitFor(() => {
    MOCK_RECOMMENDATION_SUCCESS.forEach(item => {
      ...
      expect(within(renderedItem).getByText(
        capitalize(lowerCase(item.type)))).toBeInTheDocument();
    })
  });
});
...
it('should render with correct error for 401 response', async () => {
  ...
  const { getByText } = render(<Router><Recommendation /></Router>);

  await waitFor(() => {
    expect(getByText("authorization: Authorization header was not provided"))
      .toBeInTheDocument();
  });
});
...
it('should render with correct error for 422 response', async () => {
  mock.onPost('/user').reply(200, postResp);
  mock.onGet('/recommendation').reply(422,
